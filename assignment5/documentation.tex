	...
	\item{\textsc{storeTree}}
		...
		\begin{description}
			...
			\item{Input}
				...
				A binary tree to be stores, an enumerated order \(IN\_ORDER, PRE\_ORDER, POST\_ORDER\). Also a file name
...
tests, and assignments.  The time complexity for this function is O(N*lg\^2(N)) to dequeue the entire list then
O(N*ln\^2(N)). The reason it is log squared is because the structure used to hold the nodes between 
to enqueue everything back onto it leaving the total efficiency as...
...
enqueued everything back onto it 
