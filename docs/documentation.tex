...
\begin{lstlisting}
void setImageInfo(int, int, pType);
\end{lstlisting}
...
\begin{description}
	...
	void processEntry( ImageType<pType>[], bool[],
		char[][NAME_LEN], int );
		\end{lstlisting}
	...
	void promptForLoc( const char[], ImageType<pType>&, 
			int&, int& );
		\end{lstlisting}
	...
	void fillRegs( ImageType<pType>[], bool[], 
			char[][NAME_LEN], int, char** );
		\end{lstlisting}
	...
	\item{\textsc{loadimage}}
		...
		\begin{lstlisting}
	void loadImage( ImageType<int>[],
			bool[], char[][NAME_LEN] );

	void loadImage( ImageType<rgb>[], 
			bool[], char[][NAME_LEN] );
		\end{lstlisting}
		...
		\begin{description}
			\item{Purpose}
				...
				of the .pgm or .ppm files in the local images directory (defined as a constant).
	...
	\item{\textsc{saveimage}}
		...
		\begin{lstlisting}
	void saveImage( ImageType<int>[], 
			bool[], char[][NAME_LEN] );

	void saveImage( ImageType<rgb>[], 
			bool[], char[][NAME_LEN] );
		\end{lstlisting}
	...
	\item{\textsc{getimageinfo}}
		...
		\begin{lstlisting}
	void getImageInfo( ImageType<int>[], 
			bool[], char[][NAME_LEN] );

	void getImageInfo( ImageType<rgb>[], 
			bool[], char[][NAME_LEN] );
		\end{lstlisting}
	...
	\item{\textsc{setpixel}}
		...
		\begin{lstlisting}
	void setPixel( ImageType<int>[], 
			bool[], char[][NAME_LEN] );

	void setPixel( ImageType<rgb>[], 
			bool[], char[][NAME_LEN] );
		\end{lstlisting}
		...
		\begin{description}
			\item{Purpose}
				...
				value(s) to change that pixel to.
	...
	\item{\textsc{getpixel}}
		...
		\begin{lstlisting}
	void getPixel( ImageType<int>[], 
			bool[], char[][NAME_LEN] );

	void getPixel( ImageType<rgb>[], 
			bool[], char[][NAME_LEN] );
		\end{lstlisting}
		...
		\begin{description}
			\item{Purpose}
				...
				Return the value(s) of a pixel in a selected image to the user.
	...
	\item{\textsc{extractsub}}
		...
		\begin{lstlisting}
	void extractSub( ImageType<pType>[], 
			bool[], char[][NAME_LEN] );
		\end{lstlisting}
	...
	\item{\textsc{enlargeimg}}
		...
		\begin{lstlisting}
	void enlargeImg( ImageType<pType>[], 
			bool[], char[][NAME_LEN] );
		\end{lstlisting}
	...
	\item{\textsc{shrinkimg}}
		...
		\begin{lstlisting}
	void shrinkImg( ImageType<pType>[], 
			bool[], char[][NAME_LEN] );
		\end{lstlisting}
	...
	\item{\textsc{reflectimg}}
		...
		\begin{lstlisting}
	void reflectImg( ImageType<pType>[], 
			bool[], char[][NAME_LEN] );
		\end{lstlisting}
	...
	\item{\textsc{translateimg}}
		...
		\begin{lstlisting}
	void translateImg( ImageType<pType>[],
			bool[], char[][NAME_LEN] );
		\end{lstlisting}
	...
	\item{\textsc{rotateimg}}
		...
		\begin{lstlisting}
	void rotateImg( ImageType<pType>[], 
			bool[], char[][NAME_LEN] );
		\end{lstlisting}
	...
	\item{\textsc{subtractimg}}
		...
		\begin{lstlisting}
	void subtractImg( ImageType<pType>[], 	
			bool[], char[][NAME_LEN] );
		\end{lstlisting}
	...
	\item{\textsc{negate img}}
		...
		\begin{lstlisting}
	void negateImg( ImageType<pType>[], 	
			bool[], char[][NAME_LEN] );
		\end{lstlisting}
	...
	\item{\textsc{clearregister}}

		\begin{lstlisting}
	void clearRegister( ImageType<pType>[], 	
			bool[], char[][NAME_LEN] );
		\end{lstlisting}
		...
		\begin{description}
			...
			\item{Input}
				...
				\begin{itemize}
					...
					\item{img[]}
						...
						The array with the images loaded

					\item{loaded[]}

						The bool array to set that the array is empty

					\item{name[][NAME\_LEN]}

						The array with the names of the images
			...
			\item{Assumptions}
				...
				Nothing is assumed, but it makes sense to call this only if there is something to clear.
	...
	int findLocalPPM( char **&filenames );
		...
		\begin{description}
			\item{Purpose}
				...
				A somewhat brittle function that reads all the .ppm files from a local
				directory (defined as a constant) and places them into a dynamically
				allocated c style string array.
			...
			\item{Output}
				...
				Allocates enough memory for a list of all the ".ppm" files
				in the local path specified by the FILELOC constant.  It
				then copies the file names to the array and returns the
				number of rows in the array.
	...
	void readImage(const char fname[], ImageType<int>& image)
		...
		\begin{description}
			\item{Purpose}
				...
				Reads the image (PGM or PPM) into the image object from a file
	...
	void writeImage(const char fname[], ImageType<rgb>& image)
		...
		\begin{description}
			\item{Purpose}
				...
				Writes the image (PGM or PPM) to disk
...
For the RGB class, the implementation was quite simple. All that was required was
a class with three ints, and operator overloads for $+ - * / += -= *= /= < > ==$
abs( ) (absolute value). This was overall very easy to implement and is very
intuitive and does not need to be spelled out for every function. Overall, each
operator is overloaded for ints and rgb classes. For the rgb class comparison,
each rgb value is compared against the other rgb value respectively. For the int
overload, the rgb values are averaged and then compared to the int value.
